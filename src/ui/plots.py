...
from src.ui.palette import (INSTRUMENT, TEXT_DIM, TRUTH_STYLE, WATERFALL_CMAP,
                             style_axes, tier_color)
...
def waterfall_figure(session, smoothed=True, nperseg=256):
    ...
    for event in session.emitter_events(smoothed=smoothed):
        ...
        ax.text(freqs_mhz[0], event.start_us / 1000.0, f" {event.label}",
    ...
    if session.truth:
        for seg in session.truth:
            ...
            ax.text(freqs_mhz[-1], seg.start_s * 1000.0,
