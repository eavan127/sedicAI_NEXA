...
def embed_pulse_train(pulse, pri, fs, total_duration):
    ...
    for start in range(0, max(total_samples - len(pulse), 1), pri_samples):
...
def random_radar_example(fs=None, total_duration=None, rng=None):
    ...
    pulse_width = rng.uniform(*CFG["radar"]["pulse_width_s"])
    bandwidth = rng.uniform(*CFG["radar"]["bandwidth_hz"])
    pri = rng.uniform(*CFG["radar"]["pri_s"])
    ...
    return embed_pulse_train(pulse, pri, fs, total_duration)
