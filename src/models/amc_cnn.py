...
class STFTBranch(nn.Module):
    ...
    def __init__(self, n_fft=16, hop_length=4, out_channels=64, freq_summary=False,
                 keep_rows=False, dwell_feature=False):
        super().__init__()
        self.n_fft = n_fft
        ...
        # Any of the three flags needs frequency resolution kept (not pooled
        # away) for the conv path -- freq_summary and dwell_feature read the
        # raw STFT magnitude directly regardless, but the conv features they
        # sit beside, and keep_rows's row_proj, all want the full row count.
        ...
        # freq_summary and dwell_feature -- same shape of downsampling
        # either way.
        ...
        # default 2x2 pool halves them, the freq_summary/dwell_feature
        # time-only pool keeps all.
        if keep_rows:
            n_rows = n_fft if keep_freq_res else n_fft // 2
            self.row_proj = nn.Conv1d(out_channels * n_rows, out_channels, kernel_size=1)
            self.row_bn = nn.BatchNorm1d(out_channels)

        # out_channels is fixed regardless of any flag; freq_summary adds 3
        # hand-built channels, dwell_feature adds 1 more, independently.
        # keep_rows does not change it: the learned layer maps back to
        # `out_channels`. Read by AMC_CNN to size fusion correctly instead of
        # hardcoding it.
    ...
    def forward(self, x):
        ...
        if self.freq_summary or self.keep_rows or self.dwell_feature:
...
class AMC_CNN(nn.Module):
    def __init__(self, num_classes, input_len=1024, stft_freq_summary=None,
                 stft_keep_rows=None, stft_dwell_feature=None):
        super().__init__()
