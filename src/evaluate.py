...
def evaluate(ensemble=False, n_models=5):
    ...
    def _class_mean_prob_by_snr(idx):
        ...
        very different fixes, see the SNR-declining-recall note below."""
        vals = []
        for s in unique_snrs:
            m = (snr_test == s) & (y_test[:, idx] == 1)
    ...
    # legend box on top of the QPSK/64QAM lines rising through the middle of
    # the plot. Fixed outside-right position never overlaps data, regardless
    # of how the lines move on a future rerun.
