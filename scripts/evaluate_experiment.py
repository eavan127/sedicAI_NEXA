...
def print_report(ev):
    ...
    print(f"flags: stft_freq_summary={ev['flags']['stft_freq_summary']}  stft_keep_rows={ev['flags']['stft_keep_rows']}"
          f"  stamp_branch={ev['flags'].get('stamp_branch', False)}"
...
def main():
    ...
    ap.add_argument("--stamp-branch", action="store_true", help="build the model with model.stamp_branch on")
    ...
    CFG["model"]["stamp_branch"] = bool(args.stamp_branch)
    ...
    result.update({
        ...
        "flags": {"stft_freq_summary": bool(args.stft_freq_summary), "stft_keep_rows": bool(args.stft_keep_rows),
                  "stamp_branch": bool(args.stamp_branch)},
