...
def build_data():
    ...
    # --- test split: the Performance page ---------------------------------
    X, y, snr = load_data()
    d = CFG["dataset"]
    _, _, test = stratified_split(y, snr, d["val_frac"], d["test_frac"], d["seed"])
    size = _f32(data_out / "test_X.bin", X[test])
    _f32(data_out / "test_y.bin", y[test])
    np.ascontiguousarray(snr[test], dtype="<i2").tofile(data_out / "test_snr.bin")
    (data_out / "test_split.json").write_text(json.dumps({
        "n": int(len(test)), "window_len": int(X.shape[-1]),
        "classes": list(CLASSES), "snr_bins": CFG["snr_bins_db"],
        # Recorded so the page can say WHICH dataset produced the numbers.
        # data/processed is regenerated by scripts/build_dataset.py; a smoke
        # run and a full run both land here and look identical otherwise.
        "total_windows_in_dataset": int(X.shape[0]),
    }))
    print(f"  data/test_X.bin  ({size / 1024:.0f} KB, {len(test)} windows "
          f"of {X.shape[0]} total)")
